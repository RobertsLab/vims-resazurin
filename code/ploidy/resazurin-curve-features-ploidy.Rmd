---
title: "resazurin-curve-features-ploidy"
...
This notebook applies the curve-feature comparison approach in ploidy experiment analyses. It uses the existing
`ploidy_metabolism.csv` output.

## Expected inputs

| Path | Description |
|------|-------------|
| `output/ploidy/ploidy_metabolism.csv` | Per-well per-timepoint blank-corrected and size-normalized metabolism data |

## Expected outputs

All outputs are written to
`output/ploidy/resazurin-curves/`.
...
compute_curve_features <- function(df, value_col, metric_label) {
  id_vars <- intersect(
    c("trace_id", "family_id_group", "ploidy",
...
run_feature_family_stats <- function(df) {
  df <- df %>%
    dplyr::filter(is.finite(feature_value))

  # Extract vectors to avoid *tmp* scoping issue inside group_modify
  fv <- df$feature_value
  pl <- df$ploidy

  aov_fit <- aov(fv ~ pl)
  aov_tab <- summary(aov_fit)[[1]]
  rownames(aov_tab) <- c("ploidy", "Residuals")

  ss_ploidy <- aov_tab["ploidy", "Sum Sq"]
  df_ploidy <- aov_tab["ploidy", "Df"]
  ms_error <- aov_tab["Residuals", "Mean Sq"]
  ss_total <- sum(aov_tab$`Sum Sq`, na.rm = TRUE)
  eta_sq <- ss_ploidy / ss_total
  omega_sq <- (ss_ploidy - df_ploidy * ms_error) / (ss_total + ms_error)

  fam_means <- df %>%
    group_by(ploidy) %>%
    summarise(mean = mean(feature_value), .groups = "drop")

  pair_grid <- tidyr::crossing(ploidy1 = fam_means$ploidy,
                               ploidy2 = fam_means$ploidy) %>%
    filter(as.character(ploidy1) < as.character(ploidy2)) %>%
    left_join(fam_means, by = c("ploidy1" = "ploidy")) %>%
    rename(mean1 = mean) %>%
    left_join(fam_means, by = c("ploidy2" = "ploidy")) %>%
    rename(mean2 = mean) %>%
    mutate(abs_d = abs(mean1 - mean2) / sqrt(ms_error))

  kw <- kruskal.test(fv ~ pl)

  tibble(
    n = nrow(df),
    n_family = n_distinct(df$ploidy),
    f_statistic = aov_tab["ploidy", "F value"],
    p_value = aov_tab["ploidy", "Pr(>F)"],
...
run_feature_pairwise <- function(df) {
  df <- df %>%
    filter(is.finite(feature_value), !is.na(ploidy))

  fv <- df$feature_value
  pl <- factor(df$ploidy)

  if (length(fv) < 3 || nlevels(pl) < 2) return(tibble())

  aov_fit <- aov(fv ~ pl)
  tk <- tryCatch(TukeyHSD(aov_fit)[[1]], error = function(e) NULL)
  if (is.null(tk)) return(tibble())
  as_tibble(tk, rownames = "contrast") %>%
    rename(estimate = diff, p.value = `p adj`) %>%
    mutate(SE = NA_real_, df = NA_real_, t.ratio = NA_real_) %>%
    select(contrast, estimate, SE, df, t.ratio, p.value)
}
...
```{r discover-inputs}
out_dir <- here("output/ploidy", "resazurin-curves")
fig_dir <- file.path(out_dir, "ploidy/figures")

dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(fig_dir, recursive = TRUE, showWarnings = FALSE)

metabolism_path <- here("output/ploidy", "ploidy_metabolism.csv")
...
metabolism_all <- metabolism_all %>%
  mutate(
    ...
    ploidy       = str_to_lower(trimws(ploidy))
  )
...
trajectory_features <- curve_features %>%
  count(ploidy, value_metric, family_id_group, trajectory_class, name = "n") %>%
  group_by(ploidy, value_metric, family_id_group) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup()
...
curve_feature_family_summary <- curve_feature_long %>%
  group_by(ploidy, value_metric, feature, family_id_group) %>%
  summarise(
    n = n(),
...
observational units. Features are ranked by a combined score favoring stronger ploidy effect size (`eta_sq`) and lower BH-adjusted p-value. This is intended as a screening/ranking analysis rather than a single confirmatory model. `baseline_value` is retained in the full feature table but excluded from the ranking because the goal here is to compare curve shape, capacity, responsiveness, and depression-like behavior rather than the normalized starting reference.
...
p_feature_rank <- ggplot(rank_plot_df,
  ...
  geom_col(width = 0.75) +
  facet_wrap(~ value_metric, scales = "free") +
...
best_plot_df <- curve_feature_long %>%
  semi_join(best_curve_features,
            by = c("value_metric", "feature")) %>%
  mutate(
    family_id_group = factor(
      family_id_group,
      levels = str_sort(unique(family_id_group), numeric = TRUE)
    ),
    panel_label = paste(ploidy, value_metric, feature, sep = "\n")
  )
...
curve_feature_heatmap_df <- curve_feature_family_summary %>%
  ...
  group_by(ploidy, value_metric, feature) %>%
...
p_feature_heatmap <- ggplot(curve_feature_heatmap_df,
  ...
  facet_wrap(~ ploidy, scales = "free", ncol = 2) +
...
p_trajectory <- ggplot(trajectory_plot_df,
  ...
  facet_grid(value_metric ~ ploidy, scales = "free_x", space = "free_x") +
