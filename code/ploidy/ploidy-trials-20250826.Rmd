---
title: VIMS resazurin ploidy trials - second round 20250826 
author: "AS Huffmyer"
date: '2025'
output:
...
```{r}
# Set the folder path
folder_path <- "data/ploidy/plate-files/20250826"  
...
```{r}
metadata<-read_xlsx(path="data/ploidy/metadata/ploidy_metadata_20250826.xlsx")%>%
...
```{r}
size<-read_xlsx("data/ploidy/size-images/ploidy_size_20250826.xlsx")%>%
  mutate(date=as.character(date))
```
...
Remove oysters identified as dead (likely dead before trials). 

```{r}
remove<-c("plate3 E07", "plate3 F07", "plate3 F08", "plate4 F07")
...
```{r}
full_data%>%
  ggplot(aes(x=timepoint, y=fluorescence.corr, colour=ploidy, group=interaction(date, well, plate)))+
  facet_wrap(~date)+
  geom_point()+
  geom_line()+
  theme_classic()
...
```{r}
full_data%>%
  ggplot(aes(x=timepoint, y=value, colour=ploidy, group=interaction(date, well, plate)))+
  facet_wrap(~date)+
  geom_point()+
  geom_line()+
  theme_classic()
...
leveneTest(sqrt(value) ~ ploidy * timepoint, data=full_data)
```
Significant ploidy x time effect - different metabolic rates. 
...
plot2e<-ggplot(pred_data, aes(x = timepoint, y = predicted_value_ploidy^2, color = ploidy, group = ploidy, fill = ploidy)) +
...
Output file for additional analyses. 
```{r}
full_data%>%
  select(date, plate, well, timepoint, family, ploidy, value, unique)%>%
  write_csv("output/ploidy/ploidy_metabolism.csv")
```