---
title: VIMS resazurin ploidy trials - first round 20250813
author: "AS Huffmyer"
date: '2025'
output:
...
```{r}
# Set the folder path
folder_path <- "data/ploidy/plate-files/20250813"  
...
```{r}
metadata<-read_xlsx(path="data/ploidy/metadata/ploidy_metadata_20250813.xlsx")%>%
...
```{r}
size<-read_xlsx("data/ploidy/size-images/ploidy_size_20250813.xlsx")%>%
  mutate(date=as.character(date))%>%
  select(!length.in)
```
