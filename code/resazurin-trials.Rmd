...
Remove the samples that exceeded the threshold of 3000 sometime during the trial. 
...
```{r}
full_data%>%
  select(date, plate, well, timepoint, family, temperature, phenotype, value, unique)%>%
  write_csv("output/metabolism.csv")
```
...
ggsave(plot2e, filename="figures/trials/family_model_predictions.png", width=5, height=5, units="in")
...
emm<-emmeans(model, ~phenotype|timepoint)
pairs(emm)
```
...
plot3e<-ggplot(pred_data, aes(x = timepoint, y = predicted_value_phenotype, color = phenotype, group = phenotype, fill = phenotype)) +
  scale_fill_manual(values=c("darkorange", "purple2"), name="")+
  scale_colour_manual(values=c("darkorange", "purple2"), name="")+
  stat_summary(fun = mean, geom = "line", size = 1) +
  stat_summary(
    fun.data = mean_se,
    geom = "ribbon",
...
ggsave(plot3e, filename="figures/trials/phenotype_model_predictions.png", width=5, height=5, units="in")
```
...
plot10a<-aucs%>%
  ...
  labs(title = "C", x = "Family", y = "Metabolism (AUC)") +
  theme_classic();plot10a

ggsave(plot10a, filename="figures/trials/auc_family.png", width=10, height=5, units="in")
...
plot10b<-aucs%>%
  ...
  labs(title = "D", x = "Phenotype", y = "Metabolism (AUC)") +
  ylim(6, 10)+
  theme_classic()+
  theme(legend.position="none");plot10b

ggsave(plot10b, filename="figures/trials/auc_phenotype.png", width=3, height=5, units="in")
```

Model the data. 

```{r}
model<-lmer(sqrt(AUC) ~ family + (1|plate) + (1|date), data=aucs)
...
```{r}
model<-lmer(sqrt(AUC) ~ phenotype + (1|plate) + (1|date), data=aucs)

anova(model)
qqPlot(residuals(model))

emm<-emmeans(model, ~phenotype)
pairs(emm)
```
...
Run a statistical test for this correlation. 
...
hist(model_data$AUC)
hist(model_data$`survival-high-salinity`)
  
cor.test(model_data$AUC, model_data$`survival-high-salinity`, method=c("spearman"))
...
plot16<-merged%>%
  ...
  ggplot(., aes(x = AUC, y = `survival-low-salinity`, colour=phenotype, fill=phenotype)) +
  facet_wrap(~phenotype, scales="free_x")+
  scale_colour_manual(values=c("darkorange", "purple2"), name="Selected \nPhenotype")+
  scale_fill_manual(values=c("darkorange", "purple2"), name="Selected \nPhenotype")+
  geom_point(size = 2) +  # scatter points
  geom_smooth(method = "lm", se = TRUE, linetype = "solid") +  # regression line
  labs(
    title = "E",
    x = "Metabolism (AUC)",
    y = "Predicted Survival in Low Salinity Environments"
  )+
  theme_classic();plot16

# create an annotation data frame — one row per facet
ann_df <- data.frame(
  phenotype = c("high-salinity", "low-salinity"),  # use exact facet names from your data
  label = c("p = 0.314", "p = 0.004"),
  x = c(Inf, Inf),   # top right corner
  y = c(Inf, Inf)
)

# add facet-specific text
plot16 <- plot16 +
  geom_text(
    data = ann_df,
...
ggsave(plot16, filename="figures/correlations/auc_survival-low-salinity.png", width=8, height=5, units="in")
```
...
cor.test(model_data$AUC, model_data$`survival-low-salinity`, method = c("spearman"))
...
cor.test(model_data$AUC, model_data$`weight-low-salinity`, method = c("spearman"))
...
cor.test(model_data$AUC, model_data$`weight-high-salinity`, method = c("spearman"))
