...
```{r}
# Set the folder path
folder_path <- "data/plate-files/trials"  
...
```{r}
model<-full_data%>%
  lmer((value)^(1/3) ~ family * timepoint + (1|well:plate:date) + (1|plate:date) + (1|date), data=.)
...
ggsave(plot2, filename="figures/trials/family_trajectories.png", width=8, height=8, units="in")
```
